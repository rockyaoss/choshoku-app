...
const INITIAL_ORDERS = [
  { id: 1, date: "2025-01-15", paintType: "ダイナロックⅢ", color: "ライトグレー系", volume: 3.5, cans: 2, deadline: "2025-01-17", status: "未完了", memo: "顧客：山田工業" },
  { id: 2, date: "2025-01-15", paintType: "ハイロックECO",  color: "オーシャンブルー",  volume: 0.9, cans: 3, deadline: "2025-01-16", status: "完了",   memo: "急ぎ" },
  { id: 3, date: "2025-01-16", paintType: "プロタッチ",     color: "ダークグリーン",    volume: 4.0, cans: 1, deadline: "2025-01-18", status: "未完了", memo: "" },
  { id: 4, date: "2025-01-16", paintType: "ハイロックECO",  color: "オフホワイト",      volume: 1.6, cans: 5, deadline: "2025-01-17", status: "未完了", memo: "顧客：鈴木塗装" },
];
...
function InventoryPage({ inventory }) {
  ...
  const warnCount = inventory.filter(i => i.stock >= i.minStock && i.stock < i.minStock * 1.5).length;
  const okCount   = inventory.filter(i => i.stock >= i.minStock * 1.5).length;
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {filtered.map(item => {
          ...
          const warn = !low && item.stock < item.minStock * 1.5;
          return (
            <div key={item.code} style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ fontSize: 10, color: "#475569" }}>最低 {fmtG(item.minStock)}</div>
                </div>
              </div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    {canEdit && (
                      <button onClick={() => toggleStatus(order.id)}
                        style={{ marginLeft: 12, flexShrink: 0, padding: "6px 14px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700,
...
function EditPage({ inventory, setInventory, canEdit }) {
  const [search, setSearch] = useState("");
  const [mode, setMode] = useState("使用"); // 入荷 or 使用
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 320, overflowY: "auto" }}>
        {filtered.map(item => (
          <button key={item.code} onClick={() => setSelected(item)}
            ...
            <span style={{ fontSize: 13, fontFamily: "'DM Mono', monospace", color: item.stock < item.minStock ? "#ef4444" : "#94a3b8" }}>
              {fmtG(item.stock)}
            </span>
          </button>
        ))}
  ...
  return (
    <div style={{
      ...
      {showAuth && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }} onClick={() => { setShowAuth(false); setPin(""); setPinError(false); }}>
          <div style={{ background: "#0f172a", border: "1px solid #334155", borderRadius: 16, padding: 28, width: 280 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 12, color: "#475569", marginBottom: 20 }}>PINコードを入力してください（デモ：1234）</div>
      ...
      <div style={{ flex: 1, padding: "16px 14px 80px", overflowY: "auto" }}>
        {page === 0 && <InventoryPage inventory={inventory} />}
